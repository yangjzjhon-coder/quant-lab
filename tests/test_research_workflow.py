...
def test_service_api_exposes_research_workflow_endpoints(tmp_path: Path) -> None:
    ...
    with TestClient(app_instance) as client:
        ...
        task_id = create_task.json()["task"]["id"]
        ...
        candidate_id = create_candidate.json()["candidate"]["id"]
        ...
        assert candidates.json()["candidates"][0]["candidate_name"] == "btc_eth_regime_v2"
...
def test_service_api_can_promote_top_research_candidates(tmp_path: Path) -> None:
    ...
    with TestClient(app_instance) as client:
        ...
        assert Path(payload["evaluations"][0]["backtest"]["artifacts"]["summary_path"]).exists()
